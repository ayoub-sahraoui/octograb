...
interface InputFieldProps {
  ...
  onSelectorTypeChange?: (type: 'css' | 'xpath') => void;
}
...
export const InputField: React.FC<InputFieldProps> = ({ 
  ...
  <div className="mb-4">
        ...
        <div className="flex items-center gap-2">
            ...
            {onSelectorTypeChange && (
                <div className="flex bg-slate-100 rounded p-0.5 border border-slate-200">
                    <button 
                        onClick={() => onSelectorTypeChange('css')} 
                        className={`text-[10px] px-1.5 py-0.5 rounded transition-all cursor-pointer ${selectorType !== 'xpath' ? 'bg-white shadow text-blue-600 font-medium' : 'text-slate-500 hover:text-slate-700'}`}
                        title="Use CSS Selector"
                    >CSS</button>
                    <button 
                        onClick={() => onSelectorTypeChange('xpath')} 
                        className={`text-[10px] px-1.5 py-0.5 rounded transition-all cursor-pointer ${selectorType === 'xpath' ? 'bg-white shadow text-blue-600 font-medium' : 'text-slate-500 hover:text-slate-700'}`}
                        title="Use XPath Selector"
                    >XPath</button>
                </div>
            )}
        </div>
    ...
    <div className="flex gap-2">
      <input 
        type={type} 
        className="flex-1 px-3 py-2 bg-slate-50 border border-slate-300 rounded-md text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-mono" 
        value={value || ''} 
